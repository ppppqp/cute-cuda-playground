...
Case get_case(std::string const &name) {
  if (name == "tiny")
    return {{2, 8, 2, 64, 16, 4}, 7, 49};
  if (name == "interactive")
    return {{8, 32, 8, 128, 16, 128}, 128, 2048};
  if (name == "throughput")
    return {{64, 32, 8, 128, 16, 256}, 512, 4096};
  if (name == "long")
    return {{4, 32, 8, 128, 16, 4096}, 8192, 65536};
  if (name == "ragged")
    return {{64, 32, 8, 128, 16, 1024}, 16, 16384};
  throw std::runtime_error("unknown case: " + name);
}

pa::KernelKind parse_kernel(std::string const &name) {
  if (name == "baseline")
    return pa::KernelKind::Baseline;
  if (name == "fused_v1")
    return pa::KernelKind::FusedV1;
  if (name == "vectorized_v2")
    return pa::KernelKind::VectorizedV2;
  if (name == "gqa_reuse_v3")
    return pa::KernelKind::GqaReuseV3;
  if (name == "split_kv_v4")
    return pa::KernelKind::SplitKvV4;
  throw std::runtime_error("unknown kernel: " + name);
}

void reference(pa::Problem const &p, std::vector<float> const &q, std::vector<float> const &k,
               std::vector<float> const &v, std::vector<int> const &tables,
               std::vector<int> const &lengths, std::vector<float> &out) {
  float const scale = 1.0f / std::sqrt(static_cast<float>(p.head_dim));
  std::vector<float> scores;
  for (int b = 0; b < p.batch; ++b)
    for (int h = 0; h < p.query_heads; ++h) {
...
int main(int argc, char **argv) try {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--case" && i + 1 < argc)
      ...
      fixed_context = std::stoi(argv[++i]);
    else if (arg == "--check")
      check = true;
    else if (arg == "--num-splits" && i + 1 < argc)
      num_splits = std::stoi(argv[++i]);
    else
      throw std::runtime_error(
          "usage: pa_benchmark [--case tiny|interactive|throughput|long|ragged] [--kernel "
          "baseline|fused_v1|vectorized_v2|gqa_reuse_v3|split_kv_v4] [--iterations N] [--context "
          "N] [--check]");
  }
  ...
  for (int b = 0; b < p.batch; ++b) {
    ...
    for (int j = 0; j < pages; ++j)
      tables[b * p.max_pages_per_sequence + j] = physical_pages[cursor++];
  }

  auto q = random_values<float>(p.query_elements(), 1);
  std::size_t cache_elements =
      static_cast<std::size_t>(total_pages) * p.page_size * p.kv_heads * p.head_dim;
  ...
  pa::SplitKvWorkspace workspace{nullptr, nullptr, nullptr, 0};
  // to keep the buffer from deallocated
  std::unique_ptr<pa::DeviceBuffer<float>> workspace_buffer;
  auto kind = parse_kernel(kernel_name);
  if (kind == pa::KernelKind::SplitKvV4) {
    const std::size_t states = static_cast<std::size_t>(p.batch) * p.query_heads * num_splits;
    const std::size_t partial_output_elements = states * p.head_dim;
    // partial_max + partial_sum + output for each dim
    std::size_t const workspace_elements = states + states + partial_output_elements;
    // allocate once:
    workspace_buffer = std::make_unique<pa::DeviceBuffer<float>>(workspace_elements);
    float *workspace_base = workspace_buffer->get();
    workspace.partial_max = workspace_base;
    workspace.partial_sum = workspace_base + states;
    workspace.partial_output = workspace_base + 2 * states;
    workspace.num_splits = num_splits;
  }
  dq.upload(q.data());
  dk.upload(k.data());
  dv.upload(v.data());
  dtables.upload(tables.data());
  dlengths.upload(lengths.data());
  pa::Inputs inputs{dq.get(),       dk.get(),
                    dv.get(),       dtables.get(),
                    dlengths.get(), 1.0f / std::sqrt(static_cast<float>(p.head_dim))};

  for (int i = 0; i < 10; ++i)
    pa::launch(kind, p, inputs, dout.get(), workspace);
  PA_CUDA_CHECK(cudaDeviceSynchronize());

  cudaEvent_t start, stop;
  PA_CUDA_CHECK(cudaEventCreate(&start));
  PA_CUDA_CHECK(cudaEventCreate(&stop));
  PA_CUDA_CHECK(cudaEventRecord(start));

  for (int i = 0; i < iterations; ++i)
    pa::launch(kind, p, inputs, dout.get(), workspace);
  PA_CUDA_CHECK(cudaEventRecord(stop));
  PA_CUDA_CHECK(cudaEventSynchronize(stop));
  ...
  std::cout << std::fixed << std::setprecision(3) << "case=" << case_name
            << " kernel=" << kernel_name << " mean_us=" << elapsed_ms * 1000.0f / iterations
            << '\n';

  if (check) {
    if (case_name != "tiny")
      std::cerr << "warning: CPU reference may be slow for this case\n";
    ...
    for (std::size_t i = 0; i < output.size(); ++i) {
      ...
      if (!std::isfinite(output[i]) || error > 2e-4f + 2e-4f * std::abs(expected[i]))
        ++errors;
    }
